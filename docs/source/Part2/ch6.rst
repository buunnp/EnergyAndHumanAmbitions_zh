...
2. 一个空间加热器在一个空房间的加热空气\ [#]_ 。如果房间地面面积为 10 m\ :sup:`2`，高度 2.5 m，加热器额定功率为 1,500 W，将空气温度升高 10℃ 需要多长时间？空气的密度\ [#]_ 为 1.25 kg/m\ :sup:`3`。请以分钟为单位给出近似值。

.. [#] 本题仅考虑空气，忽略其他物体——包括墙壁和家具——在实际情况中它们会显著增加所需时间。
.. [#] 利用密度来计算空气质量。


3. 早晨在 15℃ 的凉爽房间中穿衣服时，你会把衣服加热到皮肤温度（35℃）和环境温度之间的某个中间温度。\ [#]_ 如果你的衣服质量为 2 kg，需要向衣服中沉积多少能量？如果你以 100 W 的功率散热，这需要多长时间？

.. [#] 衣服的内表面接近皮肤温度，外表面接近环境温度。


4. 你弄到了一个 1 公斤的巨型墨西哥卷饼，但决定先放进冰箱稍后食用。取出来时温度为 5℃，你想在微波炉中加热到 75℃ 再吃。如果微波炉以 700 W 的速率向卷饼中输入能量，\ [#]_ 对于一个高含水量、有效比热容为 3,000 J/kg/℃ 的卷饼，需要运行微波炉多长时间？

.. [#] 注意，微波炉的额定功率可能是 1,500 W，但并非所有能量都进入卷饼，因此我们选择 700 W 以更贴近实际。


5. 假设你冬天度假回来，发现房子温度为 5℃，你想把它加热到 20℃。假设房子中包含：500 kg 的空气；\ [#]_ 1,000 kg 的家具、书籍和其他物品；以及墙壁、天花板和地板，相当于 6,000 kg 的有效\ [#]_ 质量。使用通用的比热容，这总共需要多少能量？以 10 kW 的功率加热需要多长时间？请以直观实用的单位表示，由于这是估算，可以自由取整。

.. [#] ……适用于 150 m\ :sup:`2` 占地面积
.. [#] 我们只计算外墙的一半厚度，因为它们不会从内到外全部达到室内温度。
...
8. 一栋超级保温的房子，热损失率为 100 W/℃，配备一个 5,000 W 的加热器。如果室外温度为冰点，要维持室内温度 20℃，加热器需要运行多大比例的时间？\ [#]_

.. [#] 提示：计算此情况下所需的平均功率。


9. 如果一栋房子的热损失率为 150 W/℃，在室外温度稳定为 −5℃ 的情况下，使用直接电加热\ [#]_ 将室内保持在 20℃，每天需要多少费用？电价为 $0.15/kWh。

.. [#] ……不使用热泵：直接以 100% 的效率沉积能量。


10. 请举出至少一个正文中未列出的热量转化为其他形式能量的例子。\ [#]_ 正文中我们提到了汽车上方的热空气、风、内燃机和蒸汽涡轮发电厂。
...
14. 我们可以将大气中的风视为一台巨型热机，\ [#]_ 运行在 288 K 的地表温度和对流层顶\ [#]_ 的 230 K 之间。这台热机在将太阳加热转化为气流方面的最大效率是多少？

.. [#] 而且确实如此！
.. [#] 大气风和天气被限制在大气层最低的一层——对流层中，延伸到约 12 公里的高度。


15. 既然太阳驱动地球上的能量过程，我们可以探索一个在太阳表面温度（5,800 K）和地球表面温度（288 K）之间运行的_process_的最大可能热力学效率。这个最大效率是多少？\ [#]_
...
17. 人体效率\ [#]_ 在 25% 左右，这意味着要做 100 J 的外部功，我们需要摄入 400 J 的能量含量。为了研究人体能量是否作为热机运作，请计算在热力学上要达到这一效率，冷源温度\ :math:`T_c` 需要多低。\ [#]_ 你是否认为我们的生物化学过程是作为热机运作的，还是不是？\ [#]_

.. [#] ……在将食物能量转化为有用功方面
.. [#] 热源温度\ :math:`T_h` 将是体内温度 37℃。
.. [#] 提示：我们的身体是否能经常获得这么低的温度？


18. 一罐 350 mL、温度 20℃ 的汽水\ [#]_ 被放入 EER 额定值为 10.0 的冰箱中。要从汽水中移除热能使其达到冰爽的 0℃，你需要消耗多少能量（ΔW）？

.. [#] 视为水，回想水的密度为每毫升一克。


19. 如果一台冰箱以其理论\ :math:`\epsilon_{\text{cool}}` 极限的一半运行，在 40℃ 的车库中维持 0℃ 的内部温度比在 20℃ 的室内维持同样的温度需要多消耗多少能量？这里涉及两个因素：即使效率相同，冷却能量也与 ΔT 成正比，但效率本身也会因温差翻倍而变化——双重打击。
